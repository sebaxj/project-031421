...
void global_cleanup() {
  ...
  if (g_enable_shell)
    ...
    // REFACTOR-2017 TODO: Cancel otf, le_cb threads? Does this happen in
    // ~ChucK()? things don't work so good on windows...
...
bool go(int argc, const char **argv) {
  ...
  for (i = 1; i < argc; i++) {
    if (argv[i][0] == '-' || argv[i][0] == '+' || argv[i][0] == '=' ||
      ...
      // (added 1.3.0.0)
                        ...
                        "--chugin-load:", sizeof("--chugin-load:") - 1)) {
        // get the rest
        ...
        if (arg == "off")
          ...
          // error
          CK_FPRINTF_STDERR(
              "[chuck]: invalid arguments for '--chugin-load'...\n");
      ...
      // (added 1.3.0.0)
                        ...
                        "--chugin-path:", sizeof("--chugin-path:") - 1)) {
        // get the rest
  ...
  if (!g_enable_realtime_audio && !block)
    block = TRUE;
  // audio, boost 1.4.1.0 (ge) commented out
  // if( !set_priority && !block ) g_priority = g_priority_low;
  if (!set_priority && !g_enable_realtime_audio)
    g_priority = 0x7fffffff;
  // set priority
  ...
  if (dac_name.size() > 0) {
    ...
    if (dev >= 0) {
      ...
      CK_FPRINTF_STDERR("[chuck]: unable to find dac '%s'...\n",
                        dac_name.c_str());
      CK_FPRINTF_STDERR(
          "[chuck]: | (try --probe to enumerate audio device info)\n");
      exit(1);
    }
  }
  ...
  the_chuck->setParam(CHUCK_PARAM_HINT_IS_REALTIME_AUDIO,
                      g_enable_realtime_audio);
  ...
  // Sebastian's Custom ChucK file "injection"
  ...
  EM_pushlog();
  // log
  EM_log(CK_LOG_SYSTEM, "probing '%s' audio subsystem...",
         g_enable_realtime_audio ? "real-time" : "fake-time");
  ...
  if (g_enable_realtime_audio) {
    ...
    t_CKBOOL retval =
        ChuckAudio::initialize(dac_chans, adc_chans, srate, buffer_size,
                               num_buffers, cb, (void *)the_chuck, force_srate);
    // check
    if (!retval) {
      EM_log(
          CK_LOG_SYSTEM,
  ...
  // log
  EM_log(CK_LOG_SYSTEM, "real-time audio: %s",
  ...
  if (g_enable_realtime_audio) {
    ...
    EM_log(CK_LOG_SYSTEM, "adaptive block processing: %ld",
...
#else
    g_tid_shell =
        CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)shell_cb, g_shell, 0, 0);
#endif
  }
