            ...
            double streamTime, RtAudioStreamStatus status, void * data )
{
    ...
    for( int i = 0; i < numFrames; i++ )
    {
        ...
        g_buffer[i] = input[i*MY_CHANNELS_IN];
        ...
        for( int j = 0; j < MY_CHANNELS_OUT; j++ ) { output[i*MY_CHANNELS_OUT + j] = 0; }
    }
...
int main( int argc, char ** argv )
{
    ...
    // compute
    bufferBytes = bufferFrames * GREATER(MY_CHANNELS_OUT, MY_CHANNELS_IN) * sizeof(SAMPLE);
