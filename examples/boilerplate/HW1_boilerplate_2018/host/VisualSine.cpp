...
SAMPLE
    g_stereo_buffer[SND_BUFFER_SIZE * 2]; // current stereo buffer (now playing)
GLfloat g_window[SND_BUFFER_SIZE];        // DFT transform window
GLfloat
    g_log_positions[SND_FFT_SIZE / 2]; // precompute positions for log spacing
...
void spectral_color_fall(
    double l) // RGB <- lambda l = < 380,780 > [nm] FALL TONES
{
  if (l < 380.0) {
    r = .831;
    g = .521;
...
void displayFunc() {
    ...
    glBegin(GL_LINE_STRIP);
    {
      ...
      for (i = 0; i < g_bufferSize; i++) {
        ...
        if (i < g_bufferSize / 2)
          ...
          glColor4f(.999, .999, .999,
                    1.3f - 2 * ((i - g_bufferSize / 2 + 0.01f) / g_bufferSize));
  ...
  for (i = 0; i < g_fft_size / 2; i++) {
    ...
    g_spectrums[g_wf][i].y =
        g_gain * g_freq_scale * 1.8f * ::pow(20 * cmp_abs(cbuf[i]), .5) + y;
    // increment x
    x += inc + x;
  }
  ...
  for (i = 0; i < g_depth; i++) // means go over all g_depth lines
  {
    ...
    int circle_num =
        200; // number of dots, decrease this if the computer can't handle it
    ...
    for (GLint k = 0; k < circle_num; k++) {
      ...
      for (GLint j = k * circle_piece;
           j < (k + 1) * (g_fft_size / g_freq_view) / (circle_num); j++, pt++) {
        float angle =
            (circle_num * 2 * MY_PIE * j) / (g_fft_size / g_freq_view);
        // angle = angle/12;
        float radius = sqrt(pow(g_log_positions[j] / 2048.0f, 2) +
                            pow(g_spectrums[(g_wf + i) % g_depth][j].y, 2));
        // radius = radius*4;
        //  draw the vertex
        float d = g_backwards ? g_depth - (float)i : (float)i;
        // which season is it?
        if (SEASON == 0)
          spectral_color_winter(
              400 + ((j + 1.0f) / (g_fft_size / g_freq_view) * 300.0f));
        else if (SEASON == 1)
          spectral_color_spring(
              400 + ((j + 1.0f) / (g_fft_size / g_freq_view) * 300.0f));
        else if (SEASON == 2)
          spectral_color_summer(
              400 + ((j + 1.0f) / (g_fft_size / g_freq_view) * 300.0f));
        else
          spectral_color_fall(
              400 + ((j + 1.0f) / (g_fft_size / g_freq_view) * 300.0f));
        ...
        glVertex3f(
            cos_skew * radius / (pow(1.02, k)) * cos(-angle + MY_PIE / 2) +
                (d / projection * cos(-angle + MY_PIE / 2)),
            sin_skew * radius / (pow(1.02, k)) * sin(-angle + MY_PIE / 2) +
                (d / projection * sin(-angle + MY_PIE / 2)),
